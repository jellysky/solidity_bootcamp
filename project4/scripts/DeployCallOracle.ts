import {createPublicClient, http, createWalletClient, formatEther, } from "viem";
  ...
  import { abi, bytecode, } from "../artifacts/contracts/CallOracle.sol/CallOracle.json";
  ...
  async function main() {
    ...
    const hash = await deployer.deployContract({ abi, bytecode: bytecode as `0x${string}`, args: ["0x199839a4907ABeC8240D119B606C98c405Bb0B33"], });
    ...
    const btcSpotPrice = await publicClient.readContract({address:receipt.contractAddress as `0x${string}`,abi,functionName:"getBtcSpotPrice",args:[360*60*24*90],});
