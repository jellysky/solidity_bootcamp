import { viem } from "hardhat"; // makes viem commands not require explicit abi
...
async function main() {
    ...
    const [deployer, acc1, acc2] = await viem.getWalletClients(); // functions come from viem
    ...
    // const delegateTx2 = await contract.write.delegate([acc1.account.address], {account: acc1.account,});

    const lastBlockNumber = await publicClient.getBlockNumber();
    for (let index = lastBlockNumber - 1n; index > 0n; index--) {
        const pastVotes = await contract.read.getPastVotes([acc1.account.address,index,]);
