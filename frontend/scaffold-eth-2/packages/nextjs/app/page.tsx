...
function ApiData(params: { address: `0x${string}` }) {
  return (
    <div className="card w-150 bg-primary text-primary-content mt-4">
      <div className="card-body">
        <h2 className="card-title">Retrieving token address via API</h2>
        <TokenAddressFromApi></TokenAddressFromApi>
      </div>
      <div className="card-body">
        <h2 className="card-title">Request tokens via API</h2>
        <RequestTokens address={params.address}></RequestTokens>
      </div>
      <div className="card-body">
        <h2 className="card-title">Delegate tokens via API</h2>
        <RequestDelegation address={params.address}></RequestDelegation>
      </div>
      <div className="card-body">
        <h2 className="card-title">Vote via API</h2>
        <Vote proposal={2} amount={"7"}></Vote>
      </div>
      <div className="card-body">
        <h2 className="card-title">Get votes via API</h2>
        <GetVotes address={params.address}></GetVotes>
      </div>
    </div>
...
// function TransferTokens(params: { address: string }) {
//   const [data, setData] = useState<{ result: boolean }>();
//   const [isLoading, setLoading] = useState(false);
...
//   if (isLoading) return <p>Transferring tokens via API...</p>;
//   if (!data)
//     return (
//       <div className="card w-150 bg-primary text-primary-content mt-4">
//       <div className="card-body">
...
//         <div className="form-control w-full max-w-xs my-4">
//           <label className="label">
//             <span className="label-text">Enter the address to send tokens to:</span>
//           </label>
//           <input
//             type={"text"} 
//             placeholder="Type here" 
...
//         onClick={() => {
//           setLoading(true);
//           fetch("http://localhost:3001/transfer-tokens", {
//             method: "POST",
//             headers: { "Content-Type": "application/json" },
//             body: JSON.stringify(body),
//           })
...
function RequestDelegation(params: { address: string }) {
  const [data, setData] = useState<{ result: boolean }>();
  const [isLoading, setLoading] = useState(false);

  const body = { address: params.address };

  if (isLoading) return <p>Delegating via API...</p>;
  if (!data)
    return (
      <button
        className="btn btn-active btn-neutral"
...
function GetVotes(params: { address: string }) {
  const [data, setData] = useState<{ result: boolean }>();
  const [isLoading, setLoading] = useState(false);
...
function Vote(params: { proposal: number, amount: string }) {
  ...
  const body = { proposal: params.proposal, amount: params.amount };
