...
async function initContracts() {
  const contract = await viem.deployContract("Lottery",["Lottery", "LTR", TOKEN_RATIO, parseEther(BET_PRICE), parseEther(BET_FEE)]);
  contractAddress = contract.address;
  tokenAddress = await contract.read.paymentToken(); // read token address, contract address deployed token
}
...
async function openBets(duration: string) {
  ...
  async function displayBalance(index: string) {
    ...
    const balanceBN = await publicClient.getBalance({address: accounts[Number(index)].account.address,});
